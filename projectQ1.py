...
books = pd.read_csv('BX_Books_clean.csv',delimiter=';',encoding='Latin-1',dtype={3:'object'})
ratings = pd.read_csv('BX_Book_Ratings.csv',delimiter=';',encoding='Latin-1')
users = pd.read_csv('BX_Users.csv',delimiter=';',encoding='Latin-1')
...
bookpop = bru.groupby(['ISBN'])[['Book_Rating']].count()
print(bookpop.sort_values(['Book_Rating'],ascending=False))

#author popularity = group all the books of an author and count how many have read all of them
authorpop = bru.groupby(['Book_Author'])[['Book_Rating']].count()
print(authorpop.sort_values(['Book_Rating'],ascending=False))
